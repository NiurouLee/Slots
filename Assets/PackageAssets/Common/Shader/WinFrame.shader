Shader "WinFrame"
{
    Properties
    {
        ...
        _UvMultiplier("UvMultiplier",Range(0,1)) = 2
        _Offset("Offset",Range(0,10)) = 0
        _HueOffset("HueOffset",Range(0,1)) = 0
        _RectangleArgs("RectangleArgs",Vector) = (0.3,0.3,0.1,1)
    }
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // float insideBoxSmooth(fixed2 v, fixed2 bottomLeft, fixed2 topRight) {
            //     fixed2 s = smoothstep(bottomLeft, bottomLeft + fixed2(_Gradient, _Gradient), v) -
            //         smoothstep(topRight - fixed2(_Gradient,_Gradient), topRight, v);
            //         return(s.x * s.y);
            // }
            //
            // float InsideBox(fixed2 v, fixed2 bottomLeft, fixed2 topRight) {
            //     fixed2 s = step(bottomLeft,  v) - step(topRight, v);
            //     return(s.x * s.y);
            // }
            ...
            fixed4 frag(v2f IN) : SV_Target
            {
               ...
               // clip(sy < 0);
                ...
                transformUv.x = y > 0 ? transformUv.x : 0;
