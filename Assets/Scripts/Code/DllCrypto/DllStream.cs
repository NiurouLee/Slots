...
public class DllStream : Stream
{
    ...
    private string _defaultKey = "Herjl&834_)jsH234j#@";
    ...
    public DllStream(byte[] buffer, string key)
        : this(buffer, key, true)
    {
    }

    public DllStream(byte[] buffer, string key, bool writable)
    {
    ...
    public DllStream(byte[] buffer, string key, int index, int count)
        : this(buffer, key, index, count, false)
    {
    }

    public DllStream(byte[] buffer, string key, int index, int count, bool publiclyVisible)
    {
        if (buffer == null)
    ...
    {
        if (!_isOpen) Debug.LogError("stream is closed");
        origin = _origin;
        length = _length;
    }
    ...
    public virtual int Capacity
    {
        ...
        set
        {
            ...
            if (!_isOpen) Debug.LogError("stream is closed");
            if (!_expandable && (value != Capacity)) Debug.LogError("JStream is not expandable");
    ...
    public bool CompareArray(byte[] bt1, byte[] bt2)
    {
        ...
        for (var i = 0; i < len1; i++)
        {
            if (bt1[i] != bt2[i])
            {
                // Debug.LogError($"original: {string.Join(",",bt1)}, decrypt: {string.Join(",",bt2)}\n" +
                // $"{bt1[i]} != {bt2[i]}");
                return false;
            }
        }
