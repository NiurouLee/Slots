...
namespace LitJson
{
    ...
    public class JsonMapper
    {
            ...
            if (reader.Token == JsonToken.ArrayStart)
            {
                ...
                while (true)
                {
                    ...
                    var rt = elem_type is ILRuntime.Reflection.ILRuntimeWrapperType
                        ...
                        : elem_type;
                    rt = elem_type is ILRuntime.Reflection.ILRuntimeType
                        ? ((ILRuntime.Reflection.ILRuntimeType) elem_type).ILType.TypeForCLR
                        : elem_type;
            ...
            int depth)
        {
            ...
            if (depth > max_nesting_depth)
                throw new JsonException(
                    String.Format("Max allowed object depth({1}) reached while " +
        ...
        private static void WriteValueField(object obj, JsonWriter writer,
            bool writer_is_private,
            int depth,
            IList<PropertyMetadata> listParentProperty = null, object objParent = null)
        {
            //UnityEngine.Debug.LogError($"==depth:{depth}");
            if (depth > max_nesting_depth)
                throw new JsonException(
            ...
            if (obj.GetType().ToString().Contains("ByteString"))
            {
                if (listParentProperty != null)
                {
                    foreach (var propertyMetadata in listParentProperty)
                    {
                        if (propertyMetadata.Info.Name == "Type")
                        {
                            PropertyInfo p_info = (PropertyInfo) propertyMetadata.Info;
                            object objType = p_info.GetValue(objParent, null);


                            Type objByteStringType;
                            if (obj is ILRuntime.Runtime.Intepreter.ILTypeInstance)
                            {
                                objByteStringType = ((ILRuntime.Runtime.Intepreter.ILTypeInstance) obj).Type
                                    .ReflectionType;
                            }
                            else if (obj is ILRuntime.Runtime.Enviorment.CrossBindingAdaptorType)
                            {
                                objByteStringType = ((ILRuntime.Runtime.Enviorment.CrossBindingAdaptorType) obj)
                                    .ILInstance.Type.ReflectionType;
                            }
                            else
                                objByteStringType = obj.GetType();

                            AddTypeProperties(objByteStringType);
                            IList<PropertyMetadata> propsBytString = type_properties[objByteStringType];


                            foreach (var metadata in propsBytString)
                            {
                                //bytes
                                if (metadata.Info.Name == "bytes")
                                {
                                    object objBytes = ((FieldInfo) metadata.Info).GetValue(obj);
                                    object objResult = null;

                                    if (ILRuntimeHelp.appdomain != null)
                                    {
                                        objResult = ILRuntimeHelp.appdomain.Invoke("ProtocolUtils", "GetAnyStructJson",
                                            null, new object[] {objBytes, objType});
                                    }
                                    else
                                    {
                                        //objResult = ProtocolUtils.GetAnyStructJson();

                                        var assemblyList = AppDomain.CurrentDomain.GetAssemblies();
                                        Assembly assembly = null;
                                        foreach (var ass in assemblyList)
                                        {
                                            if (ass.FullName.Contains("GameModuleAssembly"))
                                            {
                                                assembly = ass;
                                                break;
                                            }
                                        ...
                                        if (assembly != null)
                                        {
                                            var type = assembly.GetType("ProtocolUtils");
                                            var method = type.GetMethod("GetAnyStructJson",
                                                BindingFlags.Public | BindingFlags.Static);
                                            objResult = method?.Invoke(null, new object[] {objBytes, objType});
                                        }
                                    }

                                    string json = objResult as string;
                                    writer.WriteOriginString(json);
                                    return;
                                }
                            }
            ...
            if (obj is Array)
            {
                ...
                foreach (object elem in (Array) obj)
                    WriteValueField(elem, writer, writer_is_private, depth + 1);
            ...
            if (obj is IDictionary)
            {
                ...
                foreach (DictionaryEntry entry in (IDictionary) obj)
                {
                    ...
                    WriteValueField(entry.Value, writer, writer_is_private,
                        depth + 1);
                }
            ...
            foreach (PropertyMetadata p_data in props)
            {
                ...
                {
                    writer.WritePropertyName(p_data.Info.Name);
                    WriteValueField(((FieldInfo) p_data.Info).GetValue(obj),
                        writer, writer_is_private, depth + 1);
                }
                else
                {
                    ...
                    {
                        writer.WritePropertyName(p_data.Info.Name);
                        WriteValueField(p_info.GetValue(obj, null),
                            writer, writer_is_private, depth + 1, props, obj);
                    }
                }
            }
        ...
        public static List<JsonWriter> listWriterPool = new List<JsonWriter>();

        public static string ToJsonField(object obj)
        {

            JsonWriter writer = null;
            if (listWriterPool.Count > 0)
            {
                writer = listWriterPool[0];
                listWriterPool.RemoveAt(0);
            }
            else
            {
            ...
            WriteValueField(obj, writer, true, 0);
            string json = writer.ToString();
            
            writer.Reset();
            listWriterPool.Add(writer);
