...
// 
// Author : Jian.Wang
// Date : 2021/11/05/16:38
// Ver : 1.0.0
// Description : NewBieQuestModel.cs
// ChangeLog :
// **********************************************
...
{
    public class NewBieQuestModel:Model<NewbieQuest>
    {
        private List<MissionController> _currentMission;
        private uint _currentNodeIndex = 0;
        private Quest _currentQuest;
        public NewBieQuestModel()
            : base(ModelType.TYPE_NEW_BIE_QUEST)
        {
        ...
        {
            CGetNewbieQuest cGetNewbieQuest = new CGetNewbieQuest();
            var request = await APIManagerGameModule.Instance.SendAsync<CGetNewbieQuest, SGetNewbieQuest>(cGetNewbieQuest);

            if (request.ErrorCode == ErrorCode.Success)
            {
                UpdateModelData(request.Response.NewbieQuest);
            }
        }
        ...
        public Quest GetQuestNode(int index)
        {
        ...
        public override void UpdateModelData(NewbieQuest inModelData)
        {
        ...
        public ulong GetTotalPrize()
        {
            ...
            {
                if (modelData.Quests[i].Reward != null && modelData.Quests[i].Reward.Items.Count > 0)
                {
                    var coinItem = XItemUtility.GetItem(modelData.Quests[i].Reward.Items, Item.Types.Type.Coin);
                    if(coinItem != null)
                        totalReward += coinItem.Coin.Amount;
                }
            }
        ...
        protected void SetUpCurrentMissionData()
        {
            ...
            if (_currentQuest.Missions != null && _currentQuest.Missions.Count > 0)
            {
                ...
                {
                   _currentMission.Add(MissionFactory.CreateMission(_currentQuest.Missions[i]));
                }
            }
        }
    }
