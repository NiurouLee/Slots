...
namespace GameModule
{
    ...
    public class LoopListView2 : MonoBehaviour
    {
        ...
        [SerializeField] public List<ItemPrefabConfData> mItemPrefabDataList = new List<ItemPrefabConfData>();
        ...
        public void Init()
        {
            ...
            mItemPoolList = new List<ItemPool>();
            mItemPrefabDataList = new List<ItemPrefabConfData>();
            mArrangeType = ListItemArrangeType.TopToBottom;
            mItemList = new List<LoopListViewItem2>();
            mItemWorldCorners = new Vector3[4];
            mViewPortRectLocalCorners = new Vector3[4];
            mCurSnapData = new SnapData();
            mLastSnapCheckPos = Vector3.zero;
            mViewPortSnapPivot = Vector2.zero;
            mListUpdateCheckFrameCount = 0;
            mListViewInited = false;
            mLeftSnapUpdateExtraCount = 1;
            mCurSnapNearestItemIndex = -1;
            mLastFrameContainerPos = Vector3.zero;
            mSupportScrollBar = false;
            mLastItemIndex = 0;
            mLastItemPadding = 0;
            mSmoothDumpVel = 0;
            mSmoothDumpRate = 0.3f;
            mSnapFinishThreshold = 0.1f;
            mSnapVecThreshold = 145;
            mSnapMoveDefaultMaxAbsVec = 3400f;
            mIsDraging = false;
            mDistanceForRecycle0 = 300;
            mDistanceForNew0 = 200;
            mDistanceForRecycle1 = 300;
            mDistanceForNew1 = 200;
            mNeedCheckNextMinItem = true;
            mNeedCheckNextMaxItem = true;
            mCurReadyMinItemIndex = 0;
            mCurReadyMaxItemIndex = 0;
            mItemDefaultWithPaddingSize = 20;

            var eventHandler = transform.gameObject.AddComponent<DragDropEventCustomHandler>();
            eventHandler.BindingBeginDragAction(OnBeginDrag);
            eventHandler.BindingDragAction(OnDrag);
            eventHandler.BindingEndDragAction(OnEndDrag);
        }
        ...
        public SnapData mCurSnapData = new SnapData();
        ...
        */
        ...
        private Action<LoopListViewItem2> mOnRemoveItem;
        public void SetRemoveAction(Action<LoopListViewItem2> actionRemove)
        {
            mOnRemoveItem = actionRemove;
        }
        ...
        */
        ...
        public int GetViewOffset(out Vector2 offset)
        {
            ...
            if (mItemList.Count > 0)
            {
                ...
                {
                    if (mContainerTrans.anchoredPosition.x + mItemList[i].CachedRectTransform.anchoredPosition.x >
                        0)
                    {
                        offset.x = mItemList[i].CachedRectTransform.anchoredPosition.x +
                                   mContainerTrans.anchoredPosition.x;
                        offset.y = 0;
                        return mItemList[i].ItemIndex;;
                    }
                }
                // if (mContainerTrans.anchoredPosition.x < 0)
                // {
                //     itemIndex = mItemList[1].ItemIndex;
                //     offset.x = mContainerTrans.anchoredPosition.x + mItemList[0].ItemSize;
                //     offset.y = 0;
                // }
                // else
                // {
                //     itemIndex = mItemList[0].ItemIndex;
                //     offset = mItemList[0].CachedRectTransform.anchoredPosition3D;
                // }
            }
...
//            UpdateListView(viewPortSize + 100, viewPortSize + 100, newItem.ItemSize * 1.5f, newItem.ItemSize * 1.5f);
 //           AdjustPanelPos();
            ClearAllTmpRecycledItem();
            ForceSnapUpdateCheck();
            ...
            float distanceForNew1)
        {
            ...
            if (mArrangeType == ListItemArrangeType.TopToBottom)
            {
                ...
                {
                    mOnRemoveItem?.Invoke(mItemList[0]);
                ...
                {
                    mOnRemoveItem?.Invoke(mItemList[mItemList.Count - 1]);
